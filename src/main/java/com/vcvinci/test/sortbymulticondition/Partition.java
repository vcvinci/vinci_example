...
public class Partition implements Comparable<Partition>, Serializable {
    ...
    @JsonIgnore
    private transient boolean needCommit = false;
    ...
    protected Partition(final int brokerId, final int partition, final String partStr) {
        super();
    ...
    public void setNeedCommit(final boolean needCommit) {
        if (this.isAutoAck()) {
            throw new IllegalStateException("Partition is in auto ack mode");
        }
        if (this.isRollback()) {
